...
else
{
	...
	$record_sql = 'select count(distinct(role_id)) as players, mtime,count(id) as do_times , SUM(IF(((n_star - o_star) > 0),1,0)) as succ_n    FROM `'.$dbConfig['dbname'].'`.`t_log_forge` where mtime = \''.$date_int.'\' GROUP by mtime';
	...
	if(is_array($record_result) && !empty($record_result))
	{
		...
		if($db->query($sql))
		{
			echo "access do_log_forge success!";
		}
		else
		{
			echo "access do_log_forge failure!";
		}
	}
}
