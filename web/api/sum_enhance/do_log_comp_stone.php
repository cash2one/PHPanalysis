...
else
{
	...
	$record_sql = 'select count(distinct(role_id)) as players, mtime , sum(fail_n)+sum(succ_n) as do_times , sum(succ_n) as succ_n    FROM `'.$dbConfig['dbname'].'`.`t_log_comp_stone` where mtime = \''.$date_int.'\' GROUP by mtime';
	...
	if(is_array($record_result) && !empty($record_result))
	{
		...
		if($db->query($sql))
		{
			echo "access do_log_comp_stone success!";
		}
		else
		{
			echo "access do_log_conp_stone failure!";
		}
	}
}
