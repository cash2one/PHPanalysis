...
else
{
	...
	$record_sql = 'select count(distinct(role_id)) as players, mtime , count(id) as do_times , SUM(IF((succ> 0),1,0)) as succ_n    FROM `'.$dbConfig['dbname'].'`.`t_log_carve` where mtime = \''.$date_int.'\' GROUP by mtime';
	...
	if(is_array($record_result) && !empty($record_result))
	{
		...
		if($db->query($sql))
		{
			echo "access do_log_carve success!";
		}
		else
		{
			echo "access do_log_carve failure!";
		}
	}
}
