...
else
{
	...
	$record_sql = 'select count(distinct(user_id)) as players, mtime , count(id) as do_times , count(id) as succ_n    FROM `'.$dbConfig['dbname'].'`.`t_log_use_silver` where mtime >= \''.$dateStartStamp.'\' and mtime <= \''.$dateEndStamp.'\' and mtype = \'1046\'     GROUP by mtype';
	//exit("--".$record_sql);
	...
	if(is_array($record_result) && !empty($record_result))
	{
		...
		if($db->query($sql))
		{
			echo "access do_log_recycle success!";
		}
		else
		{
			echo "access do_log_recycle failure!";
		}
	}
}
