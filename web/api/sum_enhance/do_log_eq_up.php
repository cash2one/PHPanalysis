...
else
{
	...
	$record_sql = 'select count(distinct(role_id)) as players, mtime , count(id) as do_times , count(id) as succ_n    FROM `'.$dbConfig['dbname'].'`.`t_log_eq_up` where mtime = \''.$date_int.'\' GROUP by mtime';
	...
	if(is_array($record_result) && !empty($record_result))
	{
		...
		if($db->query($sql))
		{
			echo "access do_log_eq_up success!";
		}
		else
		{
			echo "access do_log_uq_up failure!";
		}
	}
}
