...
{
	$dateStart = strftime ("%Y-%m-%d", time()-(60*60*24)*2 );
}
	...
	foreach($match_data as $k => $v)
	{
		...
		{	
			...
			case 3 :
				...
				{
					$templist = explode('#',$v['goodslist']);
					$goodslist[3]['goodslist'][$k]['list'] = $templist ;
					$goodslist[3]['goodslist'][$k]['date'] =  $v['date'];
				}
				break;
			default :
				break;
...
$output = output_data($show_data,$dateStart,$dateEnd,$sum_common,$act_type);
...
$smarty->display("module/analysis/treasure_eq_log.html");




function output_data($data,$dateStart,$dateEnd,$sum_common,$act_type)  {
	global $buf_lang;
			...
			foreach($data as $k1=>$v1) {
				...
				{	 
					if($v2['date'] >= $start_temp &&  $v2['date'] <= $end_temp )
					{
						if($k2 != 'sum')
						{
							$tr .= '<tr><td>'.$k2.'</td><td>'.$v2['name'].'</td><td>'.$v2['num'].'</td><td>'.round($v2['num']/$v1['sum'],3)*100 .'%</td></tr>';//1.95583, 2)
						}
					}
				}
