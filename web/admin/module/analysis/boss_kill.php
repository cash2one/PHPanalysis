...
$sql_role = "SELECT role_id,role_name FROM db_role_base_p ";

$name_list = GFetchRowSet($sql_role);
$name_arr = array();
foreach($name_list as $name)
{
	$name_arr[$name['role_id']] = $name['role_name'];
}
$sql = "SELECT * FROM t_log_boss_kill ";
$sqlcondition = "";
$sqlcount = "";
...
if (!empty($boss_name) || !empty($map_name) || !empty($map_id) || !empty($type_id))
{
	...
	{
		if(!empty($type_id)){
			$sql_map .= " role_id='{$type_id}'";
		}
	}
	else
	{
	   if(!empty($type_id)){
			$sql_map .= " AND  role_id='{$type_id}'";
		}
	}
...
$keywordlist1 = GFetchRowSet($sqlpage);
$keywordlist =array();
foreach($keywordlist1 as $v)
{
	$v['role_name'] = $name_arr[$v['role_id']] ;
	$keywordlist[] = $v;
}
...
$role_id = trim(SS($_REQUEST['role_id']));*/
	$page = "<a href='boss_kill.php?whichpage=".$fisrt."&dateStart=".$dateStart."&dateEnd=".$dateEnd."&boss_name=".$boss_name."&map_name=".$map_id."&map_name=".$map_id."&role_id=".$role_id."'>".$buf_lang['conmon']['home_page']."

</a></font>&nbsp;&nbsp";
if($whichpage>1) {
    $page.= "<a href='boss_kill.php?whichpage=".$prev."&dateStart=".$dateStart."&dateEnd=".$dateEnd."&boss_name=".$boss_name."&map_name=".$map_id."&map_name=".$map_id."&role_id=".$role_id."'>".$buf_lang['conmon']['previous_page']."</a> ";
}
for($counter=1;$counter<=$pagecount;$counter++) {
    $page.= ("<font size=+1 color=red><a href='boss_kill.php?
...
if($whichpage < $pagecount) {
    $page.= "<a href='boss_kill.php?whichpage=".$next."&dateStart=".$dateStart."&dateEnd=".$dateEnd."&boss_name=".$boss_name."&map_name=".$map_id."&map_name=".$map_id."&type_id=".$type_id."'>".$buf_lang['conmon']['next_page']."</a> ";
}
$page.= "<a href='boss_kill.php?whichpage=".$pagecount."&dateStart=".$dateStart."&dateEnd=".$dateEnd."&boss_name=".$boss_name."&map_name=".$map_id."&map_name=".$map_id."&type_id=".$type_id."'>".$buf_lang['conmon']['last_page']."</a> ";
...
$smarty->display("module/analysis/boss_kill.html");
