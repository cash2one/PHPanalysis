...
if(!empty($match_data))
{
	foreach($match_data as $k => $v)
	{
		...
		{	
			case 1 :
				if($v['lucky'] != 0 && !empty($v['lucky_goods']) && $v['lucky_goods'] !=0 )
				{
					$templist = explode('#',$v['lucky_goods']);
...
$output = output_data($show,$dateStart,$dateEnd,$sum_common,$act_type);
...
$smarty->display("module/analysis/treasure_luckygoods_log.html");
