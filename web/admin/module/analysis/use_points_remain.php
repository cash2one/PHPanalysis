...
$sql = "SELECT p.role_id, p.role_name, b.account_name, p.level, p.points, e.last_offline_time, e.is_online".
	   " FROM db_role_attr_p as p, db_role_ext_p as e, db_role_base_p as b" .
	   " WHERE p.role_id = e.role_id AND p.role_id=b.role_id ORDER by p.points desc LIMIT 0,200";
...
$smarty->display("module/analysis/use_points_remain.html");
...
function getExcel( $alllist )
{
	...
	foreach($alllist as $k=>$v){
		...
		$excel['content'][$k][] = array('StyleID'=>'s29', 'Type'=>'String', 'content'=>$v['points']);
	}
	return $excel;
}
