...
} else if($_POST['servertxt']<>''){
    $server='sanguo'.$_POST['servertxt'];
    $server_id=$_POST['servertxt'];
}
...
if($_POST['begin_time']<>''){
    ...
    $begin_time=strtotime('2015-01-01 00:00:00');
    $end_time=strtotime(date('Y-m-d H:i:s'));
}

include_once "novice.api.php";
$sql='SELECT guide.uid,guide.condition_guide_ids,player.nick FROM guide,player WHERE  guide.uid=player.id AND player.logout_time BETWEEN '.$begin_time.' AND '.$end_time;
...
$smarty->assign($data);
$smarty->display("module/novice/role_highest_flow.html");
?>
