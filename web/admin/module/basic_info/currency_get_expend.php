...
if(isset($_POST['begin_time'])){
    ...
    //变化资源类型为：5:矿物，6:钻石，7:军令，8:金币，9:木材，20:竞技币，32:远征币，41:巅峰币，51:公会币
    $sql='SELECT role_name,role_id,account_id,item_type,action,time,left_count,count FROM xy_resourcechange WHERE '.$account_id.$role_id.$role_name.' time BETWEEN '.$begin_time.' AND '.$end_time;
    ...
    if($data_dbname){
        ...
        if($results){
            ...
            $dataList=getresults($limit_result,$item_type);
        }else{
...
function getresults($query_result,$item_type){
    if ($query_result) {
        $rows = array();
...
$smarty->assign($data);
$smarty->display("module/basic_info/currency_get_expend.html");
?>