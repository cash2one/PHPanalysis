...
$sql='SELECT normal_info FROM (SELECT * FROM xy_logoutrole ORDER BY logout_time DESC) test WHERE '.$channels.' logout_time BETWEEN '.$begin_time.' AND '.$end_time.' AND logout_time < '.$end_time.' and logout_time - create_time < 86400 GROUP BY role_id ORDER BY logout_time DESC ';
if($data_dbname){
    ...
    if($results){
      ...
      $json_data=json_encode($json_data);
        ...
        echo "<script>alert('可能是数据库变更或者找不到对应的数据库导致数据获取失败！');location.href='new_role_lost_checkpoint.php';</script>";
    }
}
...
$smarty->assign($data);
$smarty->display("module/checkpoint_lost/new_role_lost_checkpoint.html");
?>
