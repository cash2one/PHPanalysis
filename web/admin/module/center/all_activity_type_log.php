...
if (isset($act_type))
{
	if($act_type == '0')
	{
		$match_sql = "SELECT mtime,r_id,sum(players) as players,sum(do_times) as do_times " .
				     " FROM `all_routine_log` " .
				     " WHERE  mtime>='{$dateStart}' AND mtime<='{$dateEnd}'   $where " .
                                     " GROUP BY mtime, r_id $group_by".
				     " ORDER BY mtime desc";
	...
	else
	{
		$match_sql = "SELECT mtime,r_id,sum(players) as players,sum(do_times) as do_times  " .
				     " FROM `all_routine_log` " .
				     " WHERE r_id='{$act_type}' AND mtime>='{$dateStart}' AND mtime<='{$dateEnd}' $where " .
                                     " GROUP BY mtime,r_id $group_by".
				     " ORDER BY mtime desc";
		$match_data = GFetchRowSet($match_sql);
	}
...
$smarty->display("module/center/all_activity_type_log.html");
