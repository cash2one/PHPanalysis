...
echo "start exe_pay_day_all.php==".strftime("%Y-%m-%d %H:%M:%S",$now_time);
echo "\n";
...
foreach($ALL_SERVER_LIST as $k => $v)
{
	foreach($v as $kk => $vv)
	{
		if(!empty($vv['url']))
		{
			$url_list[] = $vv['url']."web/api/sh_pay_day_all.php?date={$date_str}&time={$now_time}&flag={$token}";
		}
	}
}
//$url_list[] = "http://dtz7c.my4399.com/"."web/api/sh_pay_day_all.php?date={$date_str}&time={$now_time}&flag={$token}";
...
//
echo "end exe_pay_day_all.php==".strftime("%Y-%m-%d %H:%M:%S",time());
echo "\n";
die();
