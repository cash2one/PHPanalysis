...
if($action == 0) {
    $sql_all = "select sum(account) as account,date from `all_register_db` where date>='$dStartTime' and date<='$dEndTime' group by date";
    $sql = "select sum(account) as account,date,agent_id  from `all_register_db` where date>='$dStartTime' and date<='$dEndTime' group by date,agent_id";
} else {
    $sql_all = "select sum(account) as account,date from `all_register_db` where agent_id=$action and date>='$dStartTime' and date<='$dEndTime' group by date,agent_id";
    $sql = "select account,date,server_id  from `all_register_db`  where agent_id=$action and date>='$dStartTime' and date<='$dEndTime' ";
}
...
foreach($chart_name as $key=>$value) {
    foreach($arr_date as $v) {
        $account = isset($data[$key][$v])? $data[$key][$v] : 0;
        $format_data[$key][$v] = $account;
        if($account>$format_max[$key]['max']){
            $format_max[$key]['max'] = $account;
            $format_max[$key]['date'] = $v;
        }
        $format_sum[$key] += $account;
    }
}
...
$smarty->display('module/center/day_register_count.html');
...
*/
function getChart($caption,$arr,$format_max,$sum,$count,$arr_lang) {
    ...
    $data['message'] = $caption.$arr_lang['left']['view_limit'].'——【'.$arr_lang['new']['most_top'].$arr_lang['left']['register_num'].'： <font color="#FF0000">'.$format_max['max'].$arr_lang['conmon']['ren'].'('.$format_max['date'].')</font>'.'； '.$arr_lang['new']['average'].'：<font color="#FF0000">'.round($avg_value).$arr_lang['conmon']['ren'].'</font>】';
    return $data;
}