...
$sql='SELECT role_level FROM (SELECT * FROM xy_logoutrole ORDER BY logout_time DESC) test WHERE '.$channels.' logout_time BETWEEN '.$begin_time.' AND '.$end_time.' AND logout_time < '.$lost_end_time.' and logout_time - create_time < 86400 GROUP BY role_id ORDER BY logout_time DESC ';
if($data_dbname){
...
$smarty->assign($data);
$smarty->display("module/level_lost/new_role_lost_level.html");
?>
