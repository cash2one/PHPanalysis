...
if($data_dbname){
    ...
    if($result){
        ...
        echo "<script>alert('可能是数据库变更或者找不到相应的数据库导致数据获取失败！');</script>";
    }
}
...
//格式化 数据格式:array('id'='等级','num'=>该等级流失总数 ,'rate'=>等级流失率);
function dataList($arr,$server_num){
        $row=array();
        $rows=array();
        asort($arr);
        $arr=array_count_values($arr);//统计数组中所有的值出现的次数（每个等级出现的次数)
        $keys=array_keys($arr);//获取$arr所有键名，以数字索引从新排列
        $values=array_values($arr);//获取$arr所有值，以数字索引从新排列
        for($i=0;$i<count($values);$i++){
            $rate=round(($values[$i]/$server_num)*100,2);
...
$smarty->assign($data);
$smarty->display("module/level_lost/level_lost_rate.html");
?>
