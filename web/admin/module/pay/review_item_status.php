...
$data = $log->getReviewItemLog($dateStartStamp, $dateEndStamp, $apply_admin_name, $review_admin_name, $op_id, 'item');

foreach($data as $key => $row)
{
	if($row['status'] == UNREVIEW)
	{
		$data[$key]['status_desc'] = '待审';
	}
	else if ($row['status'] == SEND_SUCCESS)
	{
		$data[$key]['status_desc'] = '成功';
	}
	else if ($row['status'] == SEND_FAIL)
	{
		$data[$key]['status_desc'] = '失败';
	}
		else if ($row['status'] == SEND_DENY)
	{
		$data[$key]['status_desc'] = '拒绝';
	}
...
$smarty->display('module/pay/review_item_status.html');
