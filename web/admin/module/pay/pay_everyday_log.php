...
//总注册
$sql_zzc = "SELECT DATE(FROM_UNIXTIME(`create_time`)) AS `date`, COUNT(account_name) AS `nums` FROM db_account_p WHERE create_time >= ".$dateStartStamp." AND create_time <= ".$dateEndStamp." and agent_id = ".$AGENT_ID." and server_id = ".$GAME_SERVER." GROUP BY DATE(FROM_UNIXTIME(`create_time`)) ASC";
$zzclist = GFetchRowSet($sql_zzc);

//新玩家数据
$sql_new = "SELECT DATE(FROM_UNIXTIME(pay_time)) AS `date`,ROUND(SUM(pay_gold),0) AS pay_day_total, COUNT(DISTINCT role_id) AS role_cnt FROM `db_pay_log_p` AS p INNER JOIN db_account_p AS a ON a.account_name = p.account_name AND a.is_new_role != 1 WHERE p.pay_time >= ".$dateStartStamp." AND p.pay_time <= ".$dateEndStamp." and p.agent_id = ".$AGENT_ID." and p.server_id = ".$GAME_SERVER." and a.agent_id = ".$AGENT_ID." and a.server_id = ".$GAME_SERVER." GROUP BY DATE(FROM_UNIXTIME(pay_time)) DESC";
...
//老玩家登陆数据
$sql_dl_old = "SELECT DATE(FROM_UNIXTIME(d.login_time)) AS `date`,COUNT(DISTINCT d.role_id) as nums FROM t_log_day_stay AS d INNER JOIN db_role_base_p AS b ON d.role_id=b.role_id INNER JOIN db_account_p AS a ON a.account_name=b.account_name WHERE  d.login_time >= ".$dateStartStamp." AND d.login_time <= ".$dateEndStamp."  AND a.is_new_role = 1 and a.agent_id = ".$AGENT_ID." and a.server_id = ".$GAME_SERVER." and b.agent_id = ".$AGENT_ID." and b.server_id = ".$GAME_SERVER." GROUP BY DATE(FROM_UNIXTIME(d.login_time)) DESC";
...
$alllist = array();
foreach($zzclist as $v){
	$array = array();
	$array["date"] = $v["date"];
	//总注册
	foreach($zzclist as $v1){
		if($v1["date"] == $v["date"]){
			$array["zzc"] = $v1["nums"];
		}
	}
...
$smarty->display("module/pay/pay_everyday_log.html");
