...
class AdminLogClass
{
	...
	public function Log($type, $detail, $number, $desc, $user_id, $user_name)
	{
		...
		$f['mtype']    = $type;
		$f['mdetail']  = $detail;
		$f['number']   = empty($number) ? 0:$number;
		if (!empty($desc))
		    $f['desc']     = $desc;
		else {
		...
		global $dbConfig;
		$sql = 'INSERT INTO `'.$dbConfig['dbname'].'`.`t_log_admin` (`admin_id`, `admin_name`, `admin_ip`, `users`, `user_name`,`mtime`,`mtype`,`mdetail`,`number`,`desc`)' .
	...
	public function getReviewLog($dateStartStamp=0, $dateEndStamp=0, $apply_admin_name='', $review_admin_name='', $status_id=0, $item_name='', $clear_type='')
	{
		...
		if ($item_name == 'item')
		{
			$sql .= " OR `item_name`='send_item' OR `item_name`='send_stone' OR `item_name`='send_equip' OR `item_name`='send_pet' OR `item_name`='send_mount' OR `item_name`='send_mount_equip' OR `item_name`='send_treasure' OR `item_name`='send_treasure_part' OR `item_name`='send_seal'";
		}
		if ($item_name)
			$sql .= ")";
		$sql .= " AND `mtype` != 2021";
	...
	public function getReviewItemLog($dateStartStamp=0, $dateEndStamp=0, $apply_admin_name='', $review_admin_name='', $status_id=0, $item_name='', $clear_type='')
	{
		...
		if ($item_name == 'item')
		{
			$sql .= " OR `item_name`='send_item' OR `item_name`='send_stone' OR `item_name`='send_equip' OR `item_name`='send_mount' OR `item_name`='send_mount_equip'";
		}
		if ($item_name)
			$sql .= ")";
		$sql .= " AND `mtype` != 2021";
