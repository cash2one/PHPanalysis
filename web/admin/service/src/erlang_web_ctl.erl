...
-spec init() -> 'ok'.
init() ->
    ets:new(erlang_web_ctl_cmds, [named_table, set, public]),
    ets:new(erlang_web_ctl_host_cmds, [named_table, set, public]),
    ok.
...
process(["status"]) ->
    ...
    case lists:keysearch(erlang_web, 1, application:which_applications()) of
        false ->
...
print_usage() ->
	...
	ets:tab2list(erlang_web_ctl_cmds),
    MaxCmdLen =
	lists:max(lists:map(
		    fun({Cmd, _Desc}) ->
			    length(Cmd)
